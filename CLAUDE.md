...
nvidiot is a REST API (FastAPI) that wraps NVIDIA Control Panel driver settings via the NVAPI library (`nvapi64.dll`). It uses ctypes FFI — no C compilation or NVIDIA SDK installation required, just an NVIDIA GPU with drivers installed.

## Commands

```bash
# Run the API server (port 8000, auto-reload)
python main.py
...
3. **`api/routes.py`** — Thin FastAPI route handlers. All route handlers delegate to `service.*` through `_handle()`, which catches `NvAPIError` and converts to HTTP 500.
...
- **Tests require a real NVIDIA GPU** — there are no mocks. Tests that modify settings (e.g., `test_round_trip_setting`) save and restore original values.
